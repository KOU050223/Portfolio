...
export default function YouTubeThumbnail({
  ...
  const handleImageLoad = () => {
    if (process.env.NODE_ENV === "development") {
      console.log(
        `[YouTubeThumbnail] 成功: ${thumbnailUrls[currentUrlIndex]} (Video ID: ${videoId})`,
      );
    }
    setImageLoaded(true);
    setImageError(false);
  ...
  if (!isValidVideoId || imageError) {
    if (process.env.NODE_ENV === "development") {
      console.log(
        `[YouTubeThumbnail] フォールバック表示: ${videoId} (エラー: ${imageError}, 無効ID: ${!isValidVideoId})`,
      );
    }
    return (
      <div
        className={`aspect-video bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-lg overflow-hidden relative flex items-center justify-center ${className}`}
      >
        <div className="text-center p-4">
          <div className="w-16 h-16 bg-red-600 rounded-full flex items-center justify-center mx-auto mb-3 shadow-lg">
            <Play className="w-8 h-8 text-white" fill="white" />
          </div>
          <p className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-1">YouTube動画</p>
          <p className="text-xs text-gray-600 dark:text-gray-400">{title}</p>
          {process.env.NODE_ENV === "development" && (
            <p className="text-xs text-red-500 mt-2">404: {videoId}</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div
      className={`aspect-video bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden relative ${className}`}
    >
      <Image
      ...
      {process.env.NODE_ENV === "development" && imageLoaded && (
        <div className="absolute top-2 right-2 bg-black bg-opacity-75 text-white text-xs px-2 py-1 rounded">
          ...
          <div>
            URL: {currentUrlIndex + 1}/{thumbnailUrls.length}
          </div>
        </div>
      )}
    </div>
